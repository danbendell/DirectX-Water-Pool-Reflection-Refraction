...
bool DebugWindowClass::Initialize(ID3D11Device* device, int screenWidth, int screenHeight, int bitmapWidth, int bitmapHeight)
{
	...
	//Initialize the vertx and index buffer
	m_previousPosX = -1;
	m_previousPosY = -1;
...
bool DebugWindowClass::InitializeBuffers(ID3D11Device* device)
{
	...
	//Load the index arrat with data
...
bool DebugWindowClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	//if it has changed thenupdate the position it is being rendered to
	...
	//Cpy the data into the vertex buffer
	memcpy(verticesPtr, (void*)vertices, (sizeof(VertexType) * m_vertexCount));

	//unlock the vertex buffer
	deviceContext->Unmap(m_vertexBuffer, 0);

	//Release the vertex array as it is not longer needed
	delete[] vertices;
	vertices = 0;
...
void DebugWindowClass::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	//Set the vertex buffer to active in the iinput assembler so it can be rendered
	deviceContext->IASetVertexBuffers(0, 1, &m_vertexBuffer, &stride, &offset);

	//Set the index buffer to active in the input assembler so it can be rendered
	deviceContext->IASetIndexBuffer(m_indexBuffer, DXGI_FORMAT_R32_UINT, 0);

	//Set the type of primitive that should be rendered from this vertex buffer in thius case triangles
