...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	//Inti the vertex and pixel shader
	result = InitializeShader(device, hwnd, L"../DirectX11Project/color.vs", L"../DirectX11Project/color.ps");
	if (!result)
	{
		return false;
	}
...
{
	//Shutdown the vertex and pizel shaders as well as the related objects
	ShutdownShader();
	return;
}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(result))
	{
		//if the shader failed to compile it should have writen something to the eerror message
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, hwnd, vsFilename);
		}
		//if there was nothing in the error messaghe then it simply could not find the shader file itseld
		else 
		{
	...
	if (FAILED(result))
	{
		...
		//If there was nothing in the error message then it simplt could not find the file itself
		else
		{
