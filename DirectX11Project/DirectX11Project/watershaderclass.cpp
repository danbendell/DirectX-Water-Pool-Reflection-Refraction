...
bool WaterShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../DirectX11Project/water.vs", L"../DirectX11Project/water.ps");
	if (!result)
	{
		return false;
	}
...
bool WaterShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
	D3DXMATRIX projectionMatrix, D3DXMATRIX reflectionMatrix, ID3D11ShaderResourceView* reflectionTexture, ID3D11ShaderResourceView* refractionTexture,
	ID3D11ShaderResourceView* normalTexture, float waterTranslation, float reflectRefractScale)
{
	bool result;

	//set the shader parameters that it will use for rendering
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, reflectionMatrix, reflectionTexture, refractionTexture, normalTexture,
	...
	//Now render the prep'd buffers with the shader
...
bool WaterShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	//This set up needs to match the vertexType strucxture in the ModelClass and in the shader
	...
	//Get a count ofg the elements in the layout
	...
	//Create a texture samplet state desc
	...
	//Setup the descc of the reflection dynamic constant buffer that is in the vertex shader
...
void WaterShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	//Set the vertex and pixel shaders that will be used to thender the triangle
	deviceContext->VSSetShader(m_vertexShader, NULL, 0);
	deviceContext->PSSetShader(m_pixelShader, NULL, 0);

	//Eet the sampler state in the pixel shader
