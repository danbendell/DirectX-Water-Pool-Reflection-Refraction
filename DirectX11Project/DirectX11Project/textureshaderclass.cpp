...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../DirectX11Project/texture.vs", L"../DirectX11Project/texture.ps");
	if (!result) 
	{
		return false;
	}
...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	//set the sahder parameters that it will use for rendering
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(result))
	{
		...
		//if there was nothing in the error message then simply could not find the shader file itself
		else
		{
	...
	//Release the vertex sahder buffer and the pixel shader buffer sincer they are no longer needed
...
void TextureShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	//pop a message on the screen to notify the user to check the text file for compile errors
...
void TextureShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	//set the vertex and pixel shaders that will be used to render the object
